...
GPIO 分配需以实际 ESP32-S3-CAM 开发板原理图为准，避免占用摄像头、PSRAM、下载串口或启动相关引脚。当前 ESP32-S3-CAM 摄像头已占用 GPIO4、GPIO5、GPIO6、GPIO7、GPIO8、GPIO9、GPIO10、GPIO11、GPIO12、GPIO13、GPIO15、GPIO16、GPIO17、GPIO18，告警外设不得复用这些引脚。示例分配如下：

| 信号 | 示例 GPIO | 方向 | 说明 |
| --- | --- | --- | --- |
| ALARM_LED | GPIO 2 | 输出 | 告警灯或状态灯。 |
| BUZZER | GPIO 14 | 输出 | 控制有源蜂鸣器。 |
| SERVO_PWM | GPIO 21 | 输出 | 舵机 PWM 控制，可选。 |
